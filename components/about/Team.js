  ...
  return (
    <section className='pt-24 px-3 sm:px-5 md:px-8 lg:px-14 xl:px-20 2xl:px-24' style={{
      ...
      <div className='max-w-screen-2xl mx-auto flex flex-col lg:flex-row justify-between lg:gap-10'>
        ...
        <div className='overflow-y-auto h-[65vh] scroll-hidden'>
          {
            teamMembers.map((member) => (
              <div className={`flex justify-between rounded-lg mb-5 ${(member.id === 1 && hoveredIndex === member.id) ? 'opacity-100' : 'opacity-60'
                } hover:opacity-100 duration-100 hover:shadow-md`}
                style={{ background: "rgba(132, 33, 192, 0.1)" }}
                key={member.id}
                onMouseEnter={() => handleMouseEnter(member.id)}
                onMouseLeave={handleMouseLeave}>
                <Image src={member.img} className='min-w-[175px]' alt='team member' />
                <div className='px-5 py-2'>
                  <div className='flex items-center justify-between mb-3'>
                    <h4 className='text-lg font-medium'>{member.name}</h4>
                    <p className='text-lg'>{member.designation}</p>
                  </div>
                  <p className='text-sm sm:text-base lg:text-sm xl:text-base'>{member.description}</p>
                </div>
              </div>
            ))
          }
